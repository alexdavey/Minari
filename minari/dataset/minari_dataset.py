...
DATASET_ID_RE = re.compile(
    r"^(?:(?P<namespace>[-_\w]*?)\/)?(?:(?P<environment>[\w]+?))?(?:-(?P<dataset>[\w:.-]+?))(?:-v(?P<version>\d+))?$"
)
...
def parse_dataset_id(dataset_id: str) -> tuple[str | None, str | None, str, int]:
    ...
    if not match:
        raise error.Error(
            f'Malformed dataset ID: {dataset_id}. (Currently all IDs must be of the form (namespace/)(env_name-)(dataset_name)-v(version). The "namespace/" is optional)'
        )
...
class MinariDatasetSpec:
    ...
    # TODO: We probably need to modify tests somewhere
