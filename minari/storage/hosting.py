...
#  TODO: Check or at least try to ensure that uploads are working correctly.
...
# Have we created the requisite subdirectories?
def download_dataset(dataset_id: str, force_download: bool = False):
    ...
    all_dataset_versions = get_remote_dataset_versions(
        download_namespace, download_env_name, download_dataset_name
    )

    dataset_versionless = gen_dataset_id(
        download_namespace, download_env_name, download_dataset_name
    )
    ...
    # TODO: Can we rename this to just namespace, env_name, etc?
